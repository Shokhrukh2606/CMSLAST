...
@section('content')
    ...
    <form method="POST" action="{{route('posts.update', [$post->id, $post->group])}}">
        ...
        @php
            $tags= \App\Post::where(array('group'=>'tags'))->get(['id','title']);
        @endphp
        <div class="form-group">
            <label for="status">Категория</label>
            <select class="form-control" id="options" name="options">
                        <option {{ $post->options==null ? 'selected' : '' }} disabled>Выбрать</option>
                        @foreach($tags as $item)
                            <option value="{{$item->id}}" {{ $post->options==$item->id ? 'selected' : '' }}>{{getLangSpec($item->title, 'ru')}}</option>
                        @endforeach
            </select>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <label for="status">Отметить как главную</label>
            <select class="form-control" id="options" name="options">
                     <option {{ $post->options==null ? 'selected' : '' }} disabled>Выбрать</option>
                    
                     <option value="yes" {{ $post->options=='yes' ? 'selected' : '' }}>Активный</option>

                <option value="no" {{ $post->options=='no' ? 'selected' : '' }}>Неактивный</option>
            </select>
        </div>
